...
function AppContent() {
  ...
  // ✅ Función para manejar input decimal con hasta 9 decimales
  const handleDecimalInput = (text, setter) => {
    const regex = /^\d*\.?\d{0,9}$/;
    if (text === '' || regex.test(text)) {
      setter(text);
    } else {
      // Si pega un número con más de 9 decimales, se redondea automáticamente
      const num = parseFloat(text);
      if (!isNaN(num)) {
        setter(num.toFixed(9).replace(/\.?0+$/, ''));
      }
    }
  };
          ...
          >
            ...
            >
              ...
              <Text style={{ color: '#ff66c4' }}>
                {parseFloat(resultadosR1.promedio).toFixed(9)}
              </Text>{' '}
              | −5%:{' '}
              <Text style={{ color: '#ff66c4' }}>
                {parseFloat(resultadosR1.promedioMinus5).toFixed(9)}
              </Text>
            </Text>
          </View>
        )}
        ...
        {rondas.map((ronda, idx) => (
          ...
          >
            ...
            >
              Promedio:{' '}
              <Text style={{ color: '#ff66c4' }}>
                {parseFloat(ronda.promedio).toFixed(9)}
              </Text>{' '}
              | −5%:{' '}
              <Text style={{ color: '#ff66c4' }}>
                {parseFloat(ronda.promedioMenos5).toFixed(9)}
              </Text>
            </Text>
          </View>
        ))}
